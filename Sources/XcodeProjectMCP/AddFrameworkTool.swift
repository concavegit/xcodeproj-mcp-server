...
public struct AddFrameworkTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        guard case let .string(projectPath) = arguments["project_path"],
              ...
              case let .string(frameworkName) = arguments["framework_name"] else {
            throw MCPError.invalidParams("project_path, target_name, and framework_name are required")
        }
        ...
        if frameworkExists {
            return CallTool.Result(
                content: [
                    .text("Framework '\(frameworkName)' already exists in target '\(targetName)'")
                ]
            )
        }
        ...
        if embed && !isSystemFramework {
            ...
            let embedBuildFile = PBXBuildFile(file: frameworkFileRef, settings: ["ATTRIBUTES": ["CodeSignOnCopy", "RemoveHeadersOnCopy"]])
