...
struct ListFilesToolTests {
    ...
    @Test func testListFilesToolCreation() {
        ...
        #expect(toolDefinition.description == "List all files in a specific target of an Xcode project")
    }
    ...
    @Test func testListFilesWithEmptyTarget() throws {
        ...
        try TestProjectHelper.createTestProjectWithTarget(name: "TestProject", targetName: "TestProject", at: projectPath)
    ...
    @Test func testListFilesWithSourceFiles() throws {
        ...
        try "// Test file content".write(to: URL(filePath: sourceFilePath), atomically: true, encoding: .utf8)
        
        let xcodeproj = try XcodeProj(path: projectPath)
        let target = xcodeproj.pbxproj.nativeTargets.first!
        let fileReference = PBXFileReference(sourceTree: .group, name: "TestFile.swift", path: "TestFile.swift")
